...
function rowAcep(but) {
    ...
    if(null == values[0] || values[0] == ''){
        ...
        editdata = {
            userCode: userCode,
            warescode: values[1],   //不可修改的框获取不到信息
            waresname: values[2],
            reserve1: values[3],
            type: type
        };
        ...
        editdata = {
            userCode: userCode,
            warescode: values[0],   //不可修改的框获取不到信息
            waresname: values[2],
            reserve1: values[3],
            type: type
        }
    ...
    myAjax(url + "api/waresManage/editWaresType",editdata);
    if(ajaxResult == '-999'){
...
function rowElim(but) {
    ...
    var popup = new Popup({
        'type': 'submit',
        'title': '',
        'text': '确定要删除商品编码为：' + values[0] + ' 的商品类型吗',
        'cancelbutton': true,
        ...
        'submitCallBack': function(){
            //alert('点击了确定');
            myAjax(url + "api/waresManage/deleteWaresType",deleData);
            if(ajaxResult == '-999'){
                window.wxc.xcConfirm("系统异常，请联系管理员");
            }else{
                var res = ajaxResult;
